...
// Lấy sản phẩm cho hero carousel
$hero_products_query = "SELECT p.*, c.name as category_name FROM products p 
                      LEFT JOIN categories c ON p.category_id = c.id 
                      ORDER BY p.id DESC LIMIT 3";  // Remove the WHERE p.featured = 1 condition
$hero_products_result = mysqli_query($conn, $hero_products_query);
$hero_products = [];

if ($hero_products_result) {
    while ($row = mysqli_fetch_assoc($hero_products_result)) {
        $hero_products[] = $row;
    }
}

// If no hero products, use default ones for fallback
if (empty($hero_products)) {
    $hero_products = [
        ['id' => 11, 'name' => 'iPhone 16 Pro Max (VN/A)', 'description' => 'Trải nghiệm hiệu năng vượt trội với chip A15 Bionic mạnh mẽ và màn hình Super Retina XDR.', 'image' => 'hero-iphone.png'],
        ['id' => 3, 'name' => 'MacBook Pro M1', 'description' => 'Sức mạnh đột phá với chip M1, thời lượng pin lên đến 20 giờ và màn hình Retina sắc nét.', 'image' => 'hero-macbook.png'],
        ['id' => 9, 'name' => 'AirPods Pro', 'description' => 'Âm thanh đỉnh cao với khả năng chống ồn chủ động và chế độ xuyên âm thông minh.', 'image' => 'hero-airpods.png']
    ];
}
    ...
    <div class="carousel-indicators">
        <?php foreach ($hero_products as $index => $product): ?>
            <button type="button" data-bs-target="#heroCarousel" data-bs-slide-to="<?php echo $index; ?>" 
                    <?php if ($index === 0) echo 'class="active" aria-current="true"'; ?> 
                    aria-label="Slide <?php echo $index + 1; ?>"></button>
        <?php endforeach; ?>
    </div>
    <div class="carousel-inner">
        <?php foreach ($hero_products as $index => $product): ?>
            <div class="carousel-item <?php if ($index === 0) echo 'active'; ?> bg-light">
                <div class="container py-5">
                    <div class="row align-items-center min-vh-50">
                        <div class="col-md-6 text-center text-md-start">
                            <h1 class="display-4 fw-bold mb-4"><?php echo $product['name']; ?></h1>
                            <p class="lead mb-4"><?php echo isset($product['description']) ? $product['description'] : substr($product['brief_description'], 0, 120); ?></p>
                            <a href="product.php?id=<?php echo $product['id']; ?>" class="btn btn-primary btn-lg rounded-pill px-4">Mua ngay</a>
                        </div>
                        <div class="col-md-6 text-center">
                            <?php if (!empty($product['image'])): ?>
                                <img src="<?php echo get_product_image_url($product['image']); ?>" alt="<?php echo $product['name']; ?>" class="img-fluid">
                            <?php else: ?>
                                <img src="assets/img/no-image.jpg" alt="<?php echo $product['name']; ?>" class="img-fluid">
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-6 mb-4 mb-md-0">
                <div class="card bg-primary text-white rounded-4 overflow-hidden position-relative h-100">
                    <div class="card-body p-4 p-md-5">
                        <h3 class="display-6 fw-bold mb-3">iPhone 16 Pro Max</h3>
...
<section class="py-5">
    <div class="container">
        <div class="text-center mb-5">
            ...
            <p class="text-muted">Đánh giá từ những khách hàng đã mua sắm tại DIENTHOAIRE</p>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-4 mb-md-0">
                <div class="card border-0 shadow-sm h-100">
                    ...
                    <div class="card-body">
                        <div class="fb-page" 
                             data-href="https://www.facebook.com/dienthoaigiakho.hcm" 
                             data-tabs="timeline" 
                             data-width="500" 
                             data-height="400" 
                            ...
                            <blockquote cite="https://www.facebook.com/dienthoaigiakho.hcm" class="fb-xfbml-parse-ignore">
                                <a href="https://www.facebook.com/dienthoaigiakho.hcm">Facebook</a>
                            </blockquote>
                        </div>
                    </div>
                </div>
...
function initMap() {
    ...
    const infowindow = new google.maps.InfoWindow({
        content: "<strong>DIENTHOAIRE</strong><br>123 Đường ABC, Quận XYZ, Hà Nội<br>Điện thoại: (84) 123 456 789",
    });
...
<script>
function initMap() {
    // Vị trí của PTIT (Học viện Công nghệ Bưu chính Viễn thông)
    const ptitLocation = { lat: 21.0466, lng: 105.7846 }; // Tọa độ của PTIT
    ...
    // Thêm info window
    const infowindow = new google.maps.InfoWindow({
        content: "<strong>DIENTHOAIRE - Chi nhánh PTIT</strong><br>Km10, Đường Nguyễn Trãi, Q.Hà Đông, Hà Nội<br>Điện thoại: (84) 123 456 789",
    });
